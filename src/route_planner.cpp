...
// TODO 4: Complete the AddNeighbors method to expand the nurrent node by adding all unvisited neighbors to the open list.
// Tips:
...
void RoutePlanner::AddNeighbors(RouteModel::Node *current_node) {
    ...
    for(RouteModel::Node *neighbor_node: current_node->neighbors){
        ...
        neighbor_node->h_value= CalculateHValue(current_node);
...
RouteModel::Node *RoutePlanner::NextNode() {
    std::sort(this->open_list.begin(), this->open_list.end(), [](const auto a_node, const auto b_node)->bool{return a_node->g_value+a_node->h_value > b_node->g_value+b_node->h_value;});
    RouteModel::Node *low_ptr=this->open_list.back();
